...
try:
    with open('score_save.txt') as score_file:
        high_score = json.load(score_file)
    print(f"Users high score is: {high_score}")
except:
...
while running:
    ...
    score = int(pygame.time.get_ticks() / 10) + (meteors_destroyed * 100)
    #Delta Time
    dt = clock.tick() / 1000
...
print(f'Exited with Code 0 in. Your score was: {int(pygame.time.get_ticks() / 10)}')

# if highscore is lower than score then it saves the score as a new highscore
if high_score < int(score):
    with open('score_save.txt', 'w') as score_file:
